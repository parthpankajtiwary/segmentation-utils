...
class SegmentationMaskVisualizer:
    ...
    def _get_agreement_disagreement(self, prediction, mask):
        """Logic for calculating agreement disagreement b/w predictions and mask"""
        prediction = np.where(prediction>=0.1, 1.0, 0.0)
        agreement_disagreement_map = np.zeros((*prediction.shape[:2], 3), dtype=np.float32)
        if len(prediction.shape)==2:
            agreement_disagreement_map[..., 1] = np.where((mask==prediction)&(mask!=0), 0.8, 0.0)
            agreement_disagreement_map[..., 0] = np.where((mask!=prediction), 0.8, 0.0)
        else:
            agreement_disagreement_map = np.where((mask==prediction)&(mask!=0.0), (0.0,0.8,0.0), (0.0,0.0,0.0))
            agreement_disagreement_map = np.where((mask!=prediction), (0.8,0.0,0.0), agreement_disagreement_map)
        return agreement_disagreement_map
    ...
    def visualize_agreement_disagreement(self, images, predictions, masks):
        ...
        fig = plt.figure(figsize=(15, 30))
        gs = gridspec.GridSpec(nrows=self.n_samples, ncols=4)
        colors = self.color_utils.create_n_hex_colors(self.n_classes)
        patches = [mpatches.Patch(color=colors[i], label=f"{self.labels[i]}") for i in range(len(self.labels))]
        ...
        for index, (image, prediction, mask) in enumerate(zip(images, predictions, masks)):
            ...
            ax1 = fig.add_subplot(gs[index, 1])
            ax2 = fig.add_subplot(gs[index, 2])
            ax3 = fig.add_subplot(gs[index, 3])

            if index == 0:
                ax0.set_title("Image", fontsize=12, weight='bold', y=1.01)
                ax1.set_title("Mask", fontsize=12, weight='bold', y=1.01)
                ax2.set_title("Prediction", fontsize=12, weight='bold', y=1.01)
                ax3.set_title("Agreement Disagreement", fontsize=12, weight='bold', y=1.01)
            ...
            l4 = ax3.imshow(image, cmap='bone')
            agreement_disagreement_map = self._get_agreement_disagreement(prediction=prediction, 
                                                                          mask=mask)
            l5 = ax3.imshow(agreement_disagreement_map, cmap=cmap[index], alpha=1)
                
            _ = [ax.set_axis_off() for ax in [ax0, ax1, ax2, ax3]]
